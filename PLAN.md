...
- **Spec ↔ lib drift engine (`scripts/spec-audit/`) — branch `EN-XX/spec-lib-drift-engine`.** Closes the Claude Design → Claude Code handoff gap at its root: the 62 hand-authored component specs drift from `@dashfi/svelte`, and Claude Design builds from the (stale) specs. The engine derives the *mechanical* spec subfields (resolved token hex, geometry, sizes, prop/variant signatures) from lib source and flags drift; prose/curation is verify-only, never overwritten. Design: `docs/superpowers/specs/2026-07-13-spec-lib-drift-engine-design.md`. **This PR:** resolver (class→token→hex w/ HSL-rounding tolerance) + tv() extractor + spec-reader + audit diff + CLI, all unit-tested (18 tests), running end-to-end over the **tv() tier** (found 10 real drifts incl. a WCAG brand-foreground dark staleness). **Remaining (tracked in PR):** static-cn / class-map / cn-conditional extractors, `buttonVariants` cross-ref resolver, subfield 3-way merge + `--write`, full 62-spec reconciliation, foundations stretch, verify-only trap guardrails. Built migration-neutral (one `DASHBOOK_LIB_ROOT` knob) so it moves into `core/packages/brand` with config only — the **handoff-routing fix** (MCP "this is a real component, import it" markers + receiving-repo skills) is the next lift, and the **core migration** is the lift after (see below).
