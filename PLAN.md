...
61 components, 6 categories. Canonical tab order:
...
**23 full-depth + 3 draft product patterns via `PatternLayout`** (was 17, all full-depth). Scaffolded as stubs on 2026-05-14 from `.knowledge/dashfi-ui-patterns.md`; Architecture (Tier 1), Shells (Tier 2), and Pages (Tier 3) all promoted to full-depth on 2026-05-14. Categories grew from 5 → 7 to host the new shapes: `Shells` (page-level chrome compositions) and `Architecture` (cross-cutting techniques) joined Data / Forms / Confirmation / States / Layout. R3 pattern extraction (2026-07-11, PR 3) added six full-depth patterns grounded in a fresh dashfi-ui gap-analysis pass (Bulk action bar, Bulk import wizard, In-dialog stepper, Card gallery, Secret reveal, Notification inbox) plus three draft stubs (Statement archive, Role permission matrix, Product catalog).
...
| Data         | Bulk action bar                      | Button, Loader, toast                                                |
| Data         | Card gallery                         | ToggleGroup, Command, Popover, Select, Input, Pagination            |
| Data         | Notification inbox                   | Select, Separator, Pill                                              |
| Data         | Statement archive (draft)            | Button, Dialog, Pill                                                  |
...
| Forms        | In-dialog stepper                    | Dialog, Skeleton                                                      |
| Forms        | Bulk import wizard                   | Dialog, Select, LinearLoader, Pill, Alert                             |
| Forms        | Role permission matrix (draft)       | Select, Checkbox, Separator                                          |
| Confirmation | Destructive confirmation             | Button, AlertDialog                                                 |
| Confirmation | Secret reveal                        | Dialog, Button                                                       |
...
| Layout       | Product catalog (draft)              | Switch, AlertDialog, Table                                            |
...
| ----- | ------- | ---------- | ----------------------------------------------------------------------------------------------------- |
| Pattern extraction R3 — product app + dash.fi sweeps | (next release) | 2026-07-11 | **PR 3 of the god-tier facelift plan** — closed gaps between the product app / dash.fi marketing site and their respective design-system coverage, grounded in two July-2026 gap-analysis passes. **Product patterns (17 → 23 full-depth + 3 draft):** six new full-depth patterns — Bulk action bar (sticky selection toolbar, async-run + progress + toast reporting, `lib/components/common/bulk-action-bar`), Bulk import wizard (template→upload→map→process Dialog + step-rail, `lib/modules/bulk-import/components/wizard/`), In-dialog stepper (steps-as-data + direction-aware fly transition, `financial-accounts/components/add-funds-dialog.svelte`), Card gallery (grid↔list + cardholder facet, `cards/overview/+page.svelte`), Secret reveal (reveal-once Dialog, `settings/developer/api-key-reveal-dialog.svelte`), Notification inbox (date-grouped feed, `notifications/all/+page.svelte`); three draft stubs (Statement archive, Role permission matrix, Product catalog) using `PatternLayout`'s `draft` prop. **Drift corrections:** workflow-data-table-page now names the dedicated Bulk action bar pattern; multi-step-onboarding gained a When-NOT-to-use note; table-filters component docs cross-link EnhancedTable's FilterConfig/FilterValues; card-detail flagged as a simplified excerpt. **Components (60 → 61):** new Sidebar component spec + docs page (the ~20-sub-component family referenced by protected-app-shell but previously undocumented); pagination docs note the PaginationWrapper convenience component. **Marketing patterns (36 → 41):** five new patterns — ComparisonTable (N-column feature matrix, replaced the standalone pricing page), DataTableBand (audit-proof data table), ChartBand (chart + copy proof section), ActionPlan (numbered recommendation list), FormCapture (inline lead-capture form) — each with a spec + docs page + `--m-*`-token-only demo. **Variants + staleness:** step-timeline (compare, pipeline-status), live-widget (term-selector, tier-slider), split-cta (dual-peer-CTA-boxes), feature-grid (4-up audience-segment), case-study-card (confidence-score + strategy-bullets) variants documented; pinned-showcase + cosmic-hero flagged "no live usage on dash.fi as of 2026-07". **Registry reconciliation:** `content/marketing.ts` rewritten against the real spec registry — removed 6 phantom slugs with no spec (centered-hero, asymmetric-product-hero, slide-frame, two-col-slide, stat-strip, cta-section), added 4 previously-undocumented specs (dark-cta-band, faq-accordion, trust-stat-band, calculator-island) plus the 5 new patterns; ⌘K now indexes exactly the shipped set. Added an "Audit agents" nav-taxonomy content note to the marketing section-rhythm foundation page. **Backlog logged:** `@dashfi/svelte` version reconciliation (dashbook `^0.5.0` vs lib `package.json` `0.4.0`); FDIC copy on `/financial-accounts` flagged for compliance review against the legal-disclosure library. `pnpm check` → 0 errors · `pnpm lint` clean · `pnpm build` succeeds. Branch `EN-XX/pattern-extraction-r3`. |
