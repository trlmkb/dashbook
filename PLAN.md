...
- No spring, no bounce
- **v1.1 facelift recipes** (portal chrome + landing page): reveal-on-scroll
  (`.db-reveal` + `use:reveal`, fade + 8px rise, once), view transitions
  (`onNavigate` + `document.startViewTransition`, 90ms out / 150ms in),
  copy-confirm (icon cross-fade + "copied" label, ~1.2s revert), hover lift
  (card/tile border + `--shadow-md`, 150ms), nav indicator (underline / left-rail
  mark scaling in from 0). All built on the existing token set — see
  `/foundations/motion` and `/brand/motion`. Every animation gated behind
...
| UI/UX facelift — micro-animations and interactions (PR 1 of 4, god-tier lift) | (staged for v1.1.0) | 2026-07-11 | **First of four independent, unmerged PRs from the god-tier facelift session** (see `docs/superpowers/specs/2026-07-11-god-tier-facelift-design.md` + `docs/superpowers/plans/2026-07-11-pr1-uiux-facelift.md`). Scope: portal chrome + landing page + shared layouts — high-end micro-animations, on-brand (3 easings, 4 durations, no spring/bounce), everything gated on `prefers-reduced-motion`. (1) **Motion utility layer** — `src/lib/utils/motion.ts` (two Svelte actions: `reveal` for fade+8px-rise-once via `IntersectionObserver`, `countUp` for 700ms ease-out-cubic numeric count-up) + `.db-reveal`/`.db-press` CSS utilities in `app.css`. (2) **View transitions** — `onNavigate` + `document.startViewTransition` in `+layout.svelte`, 90ms out / 150ms in crossfade, reduced-motion-safe (no-op fallback). (3) **Header** — sticky condensed state (backdrop blur + hairline border fade in past `scrollY > 8`) + animated active-nav underline indicator (`::after` scaleX). (4) **Landing page** — per-character clipped slide-up on the "brand & design system" heading (30ms stagger, `db-char-up`), count-up on the 4 numbers-band values, staggered `db-reveal` on every major section, palette-strip hover polish (border + hex opacity), **and fixed the stale v0.9.0 hero pill** — extracted `src/lib/content/releases.ts` as the single source for the latest release, consumed by the homepage hero pill + "Recent" band and the Footer version badge (Footer previously also hardcoded a literal, now wired to the same source). The changelog page's rich prose entries stay deliberately hardcoded (heterogeneous markup, ~17 historical entries) — instead of partial wiring (which hides drift rather than preventing it), the page carries a release-checklist comment declaring `releases.ts` the single place a new version starts. (5) **Command palette** — overlay fade + panel opacity/scale(0.95→1)/translateY(4px→0) entrance at `--dur-fast`, selection-highlight transition on tokens. (6) **Micro-interactions sweep** — CopyValue icon cross-fade + "copied" label; ThemeToggle icon rotate/fade swap; card/tile hover (border + `--shadow-md`, `.db-press` where clickable); `ArrowUpRight` nudge (translate 2px/−2px) on link hover across the dogfood link, audience tiles, and "Full changelog" link; Tabs indicator rebuilt to a persistent scaling `::after` (was static, only appeared on the active tab); Sidebar + ComponentsSidebar active-item indicator converted from a static border-color swap to a `::before` that scales in vertically; `:focus-visible` rings added where missing across Header, Tabs, Sidebar, AssetDownload, Footer, and the landing page. (7) **Docs** — new "Recipes — v1.1 facelift" table + two live `MotionPreview` demos (reveal-on-scroll, copy-confirm) added to `/brand/motion`; `/foundations/motion` usage rules gained a v1.1 recipes pointer. `pnpm check` → 0 errors (27 pre-existing warnings, unchanged). **`pnpm lint` was already failing on `main` before this PR** (345 pre-existing problems across unrelated files — verified via `git stash` diff, identical count with and without this branch's changes) — not fixed here, out of scope. Branch `EN-XX/uiux-facelift-micro-interactions`. PR created against `main`, **not merged** (user reviews). |
