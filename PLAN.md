...
> Last updated: 2026-07-11 · current version v1.0.0
>
...
| ----- | ------- | ---------- | ----------------------------------------------------------------------------------------------------- |
| Claude Code integration refresh — ARD discovery, MCP resources, plugin v0.3.0 | (plugin v0.3.0) | 2026-07-11 | **Brought the agent-facing surfaces up to July-2026 practice — plugin, MCP server, skill, connector docs.** (1) **ARD discovery** — `/.well-known/ai-catalog.json` per the draft Agentic Resource Discovery spec (ards-project/ard-spec v0.9), pointing at the MCP server, JSON API, llms.txt, and docs site. (2) **MCP resources** — `dashbook://components`, `dashbook://components/{slug}`, `dashbook://foundations/{slug}`, reusing the same spec data the `product_*` tools serve; dropped the unused `prompts` capability flag the server had been declaring without registering anything under it. (3) **Structured tool output** — `outputSchema` (zod, derived from `src/lib/specs/types.ts`) added to `product_get_component`, `product_get_token`, `product_list_components`, `marketing_get_pattern`, `marketing_get_marketing_palette`. (4) **Origin validation** on the public `/mcp` endpoint — allowlists same-origin + claude.ai/claude.com + localhost, DNS-rebinding future-proofing (server stays public/read-only, not an auth boundary). (5) **Plugin v0.3.0** — `displayName` added, `marketplace.json` gained `category`/`tags` (verified supported fields), `claude plugin validate --strict` passed, README's stale "three slash commands" claim corrected to nine. (6) **SKILL.md** — trigger-keyword sentence moved to `when_to_use` frontmatter (verified field at code.claude.com/docs/en/skills.md), `description` tightened to one sentence. (7) **Docs** — `/developers/mcp` gained an "Add as a connector" section (`claude mcp add --transport http`, claude.ai custom-connector path, ARD/llms.txt discovery) and the resources list; `/use/dev` and `/use/maintainer` updated for the new surfaces and a plugin-release checklist; `static/llms.txt` references the ARD manifest. Stayed on `@modelcontextprotocol/sdk` 1.x stable (no v2 bump — see Pending). Branch `EN-XX/claude-integration-refresh`. |
...
| **MCP SDK v2 / 2026-07-28 spec release** | dashbook / brand team | `@modelcontextprotocol/sdk` v2 and the next MCP spec revision are expected around 2026-07-28. Re-verify the stateless server pattern (`WebStandardStreamableHTTPServerTransport`) and the `registerResource`/`outputSchema` APIs used here against v2 once it's stable, then upgrade off the 1.29.x line. |
| **Submit connector to Anthropic directory** | Dash.fi (owner action, not a code task) | List the Dashbook MCP connector at claude.ai/directory now that `/mcp` has origin validation, resources, and output schemas. Requires an Anthropic-side submission — not something a code change in this repo can complete. |
