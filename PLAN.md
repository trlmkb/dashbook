...
| Marketing static JSON API | (next release) | 2026-06-27 | **Marketing parity for the static JSON API** — product had `/api/components.json` + `/api/foundations/*.json` for non-MCP clients (CI, Figma plugins, plain-`WebFetch` agents, crawlers), but marketing was MCP-only. Added 4 endpoints mirroring the MCP `marketing_*` response shapes, CORS-open + 24h-cached + `prerender=false`, same headers/404-with-hint pattern as the product endpoints: `/api/marketing/patterns.json` (catalogue, 36) + `/api/marketing/patterns/[slug].json` (full `MarketingPatternSpec` — recipe, DOM, token roles, dimensions, variants, props, gotchas; mirrors `marketing_get_pattern`) + `/api/marketing/foundations.json` (5) + `/api/marketing/foundations/[slug].json` (full foundation; valid slugs tokens/typography/layout/section/motion). Discoverability: added both to `/developers/install` "Assets you can fetch directly". Verified live: all 4 → 200 + correct shape, unknown slug → 404 with hint. `pnpm check` → 0 errors. Closes accessibility gap #1 (the marketing design system is now reachable via MCP **and** plain HTTP JSON). Remaining: `/llms.txt` root index (gap #2). Branch `EN-XX/marketing-json-api`. |
...
| **Plugin org-wide publish** | dashfi admin (Anthropic Console) | Register the `dashbook` plugin from the `trlmkb/dashbook` marketplace in **Anthropic Console → workspace → Customizations / Plugins / Marketplaces** so every Dash.fi workspace member sees it under **Claude Desktop → Customize → Plugins → Your organization** and installs in one click (no manual `claude_desktop_config.json` editing). Requires Team or Enterprise plan. Procedure documented at `/developers/install` (Path 1). Public install paths via marketplace (`/plugin marketplace add trlmkb/dashbook`) and manual MCP config are already live as fallbacks. |
| **Credit card art configurator** | brand team (content) + dashbook (build) | Add a `/brand/card` page using the same `AssetConfigurator` pattern as `/brand/logo` — pick a card variant, preview the composited card art, download MDES-spec PNG + press SVG. Integrate into MCP via `marketing_get_card_art` + `marketing_list_card_presets`, mirroring the `marketing_get_logo` shape. Triggered by the 2026-05-14 brand-team note: card design refresh + 6 new BINs being submitted for Mastercard MDES tokenization → align the refresh with MDES asset prep. Source artifacts (background + icon + logo + 1×1 issuer-hide pixel + MC MDES requirements) provided by brand. Full design + build notes at `.knowledge/credit-card-art.md`. Open inputs from brand listed there. **Server-side PNG output landed 2026-05-14 via `@resvg/resvg-js` — see `.knowledge/server-side-png.md`.** |
