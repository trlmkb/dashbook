...
- **Spec ↔ lib drift engine (`scripts/spec-audit/`) — branch `EN-XX/spec-lib-drift-engine`, PR #17.** Full standalone implementation and reconciliation complete. The CLI now audits the product foundation plus every registered shared-component spec against the rendered theme and recursively inventories library source classes/tokens. `--write` safely canonicalizes only resolved `TokenRef` light/dark literals; prose and curation remain untouched. The one-time pass canonicalized **151 resolved token occurrences**, reconciled verified anatomy drift (Card, Loader, MultiSelect, PhoneInput, DateRangeSelector, DropdownMenu, Calendar, Command, Tooltip, EnhancedTable, Pagination), and added the missing PaginationWrapper spec/page. Current safe gate: **0 stale resolved values across 61 components**; source-token coverage and inherited-dimension traces remain advisory (`--strict` gates them) until dependency-aware extraction is added. Sidebar is intentionally left to existing PR #16. The same PR closes the handoff-routing gap with machine-readable implementation/reuse metadata in MCP + JSON, exact-import requirements for Svelte, and `product_port_to` guidance for non-Svelte stacks. Built migration-neutral via `DASHBOOK_LIB_ROOT`; no core/Nx migration is performed here, but the gate can move beside the shared library in the next lift. **Ship prep (2026-07-16):** coordinated version bump — mcp-server v0.4.0, plugin v0.3.0, site v1.1.0 (CHANGELOG `[1.1.0]`). The `implementation` routing was verified live on all four #17 surfaces (`product_get_component`, `product_list_components`, `/api/components.json`, `/api/components/{slug}.json`) — identical object, 0 stale audit values, dark `--color-brand-foreground` reconciled to `#000000`. The fifth surface (`dashbook://` resources) is a #15 addition; #15 was updated to carry the same routing + de-conflicted versions (plugin v0.4.0 / mcp-server v0.5.0) and must rebase onto merged #17 to build. Merge gated on owner B4 (fresh-client reuse) sign-off.
