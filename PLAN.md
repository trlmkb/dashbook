...
| **Plugin org-wide publish** | dashfi admin (Anthropic Console) | Register the `dashbook` plugin from the `dashfi/dashbook` marketplace in **Anthropic Console → workspace → Customizations / Plugins / Marketplaces** so every Dash.fi workspace member sees it under **Claude Desktop → Customize → Plugins → Your organization** and installs in one click (no manual `claude_desktop_config.json` editing). Requires Team or Enterprise plan. Procedure documented at `/developers/install` (Path 1). Public install paths via marketplace (`/plugin marketplace add dashfi/dashbook`) and manual MCP config are already live as fallbacks. |
| **Credit card art configurator** | brand team (content) + dashbook (build) | Add a `/brand/card` page using the same `AssetConfigurator` pattern as `/brand/logo` — pick a card variant (ink / paper / brand themes), preview the rendered card art, download SVG / PNG. Integrate into MCP via a `get_card_art` or `marketing_get_card_art` tool that returns inline SVG + URL per preset, mirroring the `marketing_get_logo` shape. Per migration plan in §12 (decided 2026-05-12), the source-of-truth path is for this to live inside `core/packages/brand/` so card art renders against the same React `CreditCard` component used in product surfaces. Needs: (1) brand spec for the variants we support, (2) where the canonical card art SVG / generator lives (likely already drafted in core or the Self-Service-Onboarding prototype's `credit-card.jsx` — confirm with brand team), (3) marketing-side decision on whether last-4 and cardholder name are placeholders or omitted on the press version. |
