...
| **MCP server Phase 2** | dashbook / brand team | Foundations deep-extract beyond `tokens.ts` (typography type scale, spacing scale). Marketing tool full implementation — `marketing_get_partner_kit` per-partner asset bundles, `marketing_search` across voice / palette / disclosures. Auth gating for partner-specific assets (FDIC partner data, exec bios) — public assets stay public. End-to-end test from Claude Desktop + Claude Code against the live remote URL. |
